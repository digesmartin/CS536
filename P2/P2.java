...
/**
 * This program is to be used to test the cflat scanner. This version is set up
...
public class P2 {
    ...
    private static void testFile(String testFileName) throws IOException {
        CharNum.num = 1;
        // open input and output files
        FileReader inFile = null;
        PrintWriter outFile = null;
        try {
            inFile = new FileReader(testFileName + ".in");
            outFile = new PrintWriter(new FileWriter(testFileName + ".out"));
        } catch (FileNotFoundException ex) {
            System.err.println("File allTokens.in not found.");
            System.exit(-1);
        } catch (IOException ex) {
        ...
        Symbol my_token = my_scanner.next_token();
        while (my_token.sym != sym.EOF) {
            TokenVal tokenVal = (TokenVal) my_token.value;
            outFile.print(tokenVal.linenum + ", " + tokenVal.charnum + " ");
            ...
            switch (my_token.sym) {
                ...
                case sym.RETURN:
                    ...
                    break;
                case sym.ID:
                    outFile.println("id: " + ((IdTokenVal) my_token.value).idVal);
                    break;
                case sym.INTLITERAL:
                    outFile.println("intliteral: " + ((IntLitTokenVal) my_token.value).intVal);
                    break;
                case sym.STRINGLITERAL:
                    outFile.println("stringliteral: " + ((StrLitTokenVal) my_token.value).strVal);
                    break;
                case sym.LCURLY:
                    outFile.println("{");
                    break;
